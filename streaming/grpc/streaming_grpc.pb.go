...
const (
	Chat_ChatStream_FullMethodName = "/streaming.Chat/ChatStream"
)
...
type ChatClient interface {
	// Биде·ре·кциональный стриминг (двусторонний поток)
...
func (c *chatClient) ChatStream(ctx context.Context, opts ...grpc.CallOption) (Chat_ChatStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Chat_ServiceDesc.Streams[0], Chat_ChatStream_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
...
func (x *chatChatStreamClient) Send(m *Message) error {
	return x.ClientStream.SendMsg(m)
}

func (x *chatChatStreamClient) Recv() (*Message, error) {
	m := new(Message)
...
func (x *chatChatStreamServer) Send(m *Message) error {
	return x.ServerStream.SendMsg(m)
}

func (x *chatChatStreamServer) Recv() (*Message, error) {
	m := new(Message)
